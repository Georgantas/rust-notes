...
fn read_and_validate(b: &mut dyn io::BufRead) -> Result<PositiveNonzeroInteger, Box<dyn error::Error>> {
    ...
    b.read_line(&mut line)?; // could return io::Error
    let num: i64 = line.trim().parse()?; // could return num::ParseIntError
    let answer = PositiveNonzeroInteger::new(num)?; // could return CreationError
    Ok(answer)
}
